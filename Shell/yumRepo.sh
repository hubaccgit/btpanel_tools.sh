...
export PATH
get_Yum_Repo(){
	nodes=(http://mirrors.aliyun.com/repo/Centos-7.repo http://mirrors.163.com/.help/CentOS7-Base-163.repo https://mirrors.tuna.tsinghua.edu.cn/help/centos/index.html http://mirrors.cloud.tencent.com/repo/centos7_base.repo https://mirrors.huaweicloud.com/repository/conf/CentOS-7-anon.repo )
	i=1
	for node in ${nodes[@]}; 
	do
		start=$(date +%s.%N)
